<div class="inventory">
    <div class="inn">
        <div id="itemTooltip">
            <div class="inn">
                <span class="line">Iron Sword</span>
                <span class="line empty"></span>
                <span class="line bonus">+6 Attack Damage</span>
            </div>
        </div>
        <div id="armorSlots">
            <h1>Armor</h1>
            <div class="cell helmet">
                <!--<img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />-->
                <span class="amount"></span>
            </div>
            <div class="cell chestplate">
                <!--<img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />-->
                <span class="amount"></span>
            </div>
            <div class="cell leggings">
                <!--<img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />-->
                <span class="amount"></span>
            </div>
            <div class="cell boots">
                <!--<img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />-->
                <span class="amount"></span>
            </div>
        </div>
        <div id="mainInventorySlots">
            <h1>Inventory</h1>
            <div class="row">
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/5/58/Grid_Minecart.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/5/58/Grid_Minecart.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/5/58/Grid_Minecart.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/5/58/Grid_Minecart.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
            </div>
            <div class="row">
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
            </div>
            <div class="row">
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
            </div>
            <div class="row hotbarSeparator"></div>
            <div id="hotbarSlots">
                <div class="row hotbar">
                    <div class="cell">
                        <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                        <span class="amount"></span>
                    </div>
                    <div class="cell">
                        <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                        <span class="amount"></span>
                    </div>
                    <div class="cell">
                        <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                        <span class="amount"></span>
                    </div>
                    <div class="cell">
                        <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                        <span class="amount"></span>
                    </div>
                    <div class="cell">
                        <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                        <span class="amount"></span>
                    </div>
                    <div class="cell">
                        <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                        <span class="amount"></span>
                    </div>
                    <div class="cell">
                        <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                        <span class="amount"></span>
                    </div>
                    <div class="cell">
                        <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                        <span class="amount"></span>
                    </div>
                    <div class="cell">
                        <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                        <span class="amount"></span>
                    </div>
                </div>
            </div>
        </div>
    
        <div id="enderChestSlots">
            <h1>Ender Chest</h1>
            <div class="row">
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/5/58/Grid_Minecart.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/5/58/Grid_Minecart.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/5/58/Grid_Minecart.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/5/58/Grid_Minecart.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/2/27/Grid_Block_of_Emerald.png" />
                    <span class="amount">64</span>
                </div>
            </div>
            <div class="row">
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount">64</span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount">64</span>
                </div>
            </div>
            <div class="row">
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/3/32/Grid_Torch.png" />
                    <span class="amount"></span>
                </div>
                <div class="cell">
                    <img src="http://hydra-media.cursecdn.com/minecraft.gamepedia.com/1/13/Grid_Shears.png" />
                    <span class="amount"></span>
                </div>
            </div>
        </div>
    </div>
    
</div>